...
func addFlagsOsConstrained(cmd *cobra.Command) {
	...
	cmd.Flags().BoolVarP(&useSvcMgr, serviceManagerFlag, "w", false, "Decides which Windows Control Manager to use. Default is false / \"Windows Management Instrumentation (wmi)\". Set to true to use \"Windows Control Manager\"")
}
